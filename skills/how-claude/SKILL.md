...
Spawn a single agent that explores and explains in one pass using the Agent tool:
...
Once all explorers have returned, spawn a single agent to synthesize their findings into one coherent explanation using the Agent tool:
...
After the explanation is complete, spawn architectural critics using the Agent tool. Launch all in a single message:

| Subagent | Model | Rationale |
|----------|-------|-----------|
| Critic A | `opus` | Deep architectural analysis |
| Critic B | `sonnet` | Diverse perspective, fast |
| Critic C | `haiku` | Lightweight third perspective |
